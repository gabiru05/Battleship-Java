 ...
 * @author kenet
 */
<<<<<<< Updated upstream:src/nombreusuario/game.java
public class game extends javax.swing.JFrame {
=======
public class Player1 extends javax.swing.JFrame {
>>>>>>> Stashed changes:src/nombreusuario/Player1.java

    addSound add_sound_funciones = new addSound();
    /**
...
>>>>>>> Stashed changes:src/nombreusuario/Player1.java
        ConfirmarPLayout.setVerticalGroup(
                ConfirmarPLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, ConfirmarPLayout.createSequentialGroup()
                                .addGap(0, 0, Short.MAX_VALUE)
                                .addComponent(Confirmar, javax.swing.GroupLayout.PREFERRED_SIZE, 40,
        ...
        Juego.add(Music_icono, new org.netbeans.lib.awtextra.AbsoluteConstraints(480, 110, 60, 50));
...
>>>>>>> Stashed changes:src/nombreusuario/Player1.java
    ...
    private void Music_botonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_Music_botonActionPerformed
        ...
        if (Music_boton.isSelected()) {
            ...
            add_sound_funciones.SetMusic_icon(Music_icono, "/IMAGENES/music_on.png");
            System.out.println("Icono on");
            Music_icono.repaint();

            // restart sound
            add_sound_funciones.Setsound();

        } else {
            add_sound_funciones.SetMusic_icon(Music_icono, "/IMAGENES/music_off.png");
            System.out.println("Icono of");
            Music_icono.repaint();

            // Stop sound

            add_sound_funciones.Sound.stop();
...
=======
     ...
     * labels[j][i].addMouseListener(new MouseAdapter() {
     * boolean isVertical = false;
     * int direction = 0; // Agregamos una variable para rastrear la dirección del
     * rectángulo
     * 
     * @Override
     * public void mouseClicked(MouseEvent e) {
     ...
     * ConfirmarP.setVisible(true);
     * boolean canDraw = false;
     * for (int d = 0; d < 4; d++) { // Verificamos si el rectángulo se puede
     * dibujar en cada una de las cuatro direcciones
     * direction = (direction + 1) % 4; // Cambiamos la dirección cada vez que se
     * verifica una nueva dirección
     * canDraw = true;
     * if (direction == 0 || direction == 2) { // Verificamos si la dirección es
     * hacia la derecha o hacia la izquierda
     * for (int i = 0; i < size; i++) {
     * int newX = x + i;
     * if (direction == 2) newX = x - i; // Si la dirección es hacia la izquierda,
     * restamos en lugar de sumar
     * if (newX > 9 || newX < 1) { // Si el nuevo valor de X está fuera de los
     * límites, no dibujamos el rectángulo
     * 
     * canDraw = false;
     * break;
     * }
     ...
     * for (int i = 0; i < size; i++) {
     * int newY = y + i;
     * if (direction == 3) newY = y - i; // Si la dirección es hacia arriba,
     * restamos en lugar de sumar
     * if (newY > 9 || newY < 1) { // Si el nuevo valor de Y está fuera de los
     * límites, no dibujamos el rectángulo
     * canDraw = false;
     * break;
     * }
     ...
     * }
     * }
     * if (canDraw) break; // Si encontramos una dirección en la que el rectángulo
     * se puede dibujar, salimos del bucle
     * }
     * if (canDraw) {
     * if (direction == 0 || direction == 2) { // Dibujamos el rectángulo hacia la
     * derecha o hacia la izquierda
     * for (int i = 0; i < size; i++) {
     * int newX = x + i;
     * if (direction == 2) newX = x - i; // Si la dirección es hacia la izquierda,
     * restamos en lugar de sumar
     * if (newX > 9 || newX < 1) continue; // Si el nuevo valor de X está fuera de
     * los límites, no dibujamos esa celda
     ...
     * if (direction == 3) newY = y - i; // Si la dirección es hacia arriba,
     * restamos en lugar de sumar
     * if (newY > 9 || newY < 1) continue; // Si el nuevo valor de Y está fuera de
     * los límites, no dibujamos esa celda
            ...
            for (int j = 1; j <= 9; j++) {
                ...
                labels[j][i].addMouseListener(new MouseAdapter() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        ConfirmarP.setVisible(true);
                        boolean canDraw = false;
                        for (int d = 0; d < 4; d++) { // Verificamos si el rectángulo se puede dibujar en cada una de
                                                      // las cuatro direcciones
                            direction = (direction + 1) % 4; // Cambiamos la dirección cada vez que se verifica una
                                                             // nueva dirección
                            canDraw = true;
                            if (direction == 0 || direction == 2) { // Verificamos si la dirección es hacia la derecha o
                                                                    // hacia la izquierda
                                for (int i = 0; i < size; i++) {
                                    int newX = x + i;
                                    if (direction == 2)
                                        newX = x - i; // Si la dirección es hacia la izquierda, restamos en lugar de
                                                      // sumar
                                    if (newX > 9 || newX < 1) { // Si el nuevo valor de X está fuera de los límites, no
                                ...
                                for (int i = 0; i < size; i++) {
                                    ...
                                    if (newY > 9 || newY < 1) { // Si el nuevo valor de Y está fuera de los límites, no
                                                                // dibujamos el rectángulo
                                        canDraw = false;
                                        break;
                                    }
                            ...
                            if (canDraw)
                                break; // Si encontramos una dirección en la que el rectángulo se puede dibujar,
                                       // salimos del bucle
                        }
                        if (canDraw) {
                            if (direction == 0 || direction == 2) { // Dibujamos el rectángulo hacia la derecha o hacia
                                                                    // la izquierda
                                for (int i = 0; i < size; i++) {
                                    int newX = x + i;
                                    if (direction == 2)
                                        newX = x - i; // Si la dirección es hacia la izquierda, restamos en lugar de
                                                      // sumar
                                    if (newX > 9 || newX < 1)
                                        continue; // Si el nuevo valor de X está fuera de los límites, no dibujamos esa
                                                  // celda
                                    if (labels[newX][y] != null) {
                                        labels[newX][y].setOpaque(true);
                                        labels[newX][y].setBackground(Color.RED);
                                ...
                                for (int i = 0; i < size; i++) {
                                    ...
                                    if (direction == 3)
                                        newY = y - i; // Si la dirección es hacia arriba, restamos en lugar de sumar
                                    if (newY > 9 || newY < 1)
                                        continue; // Si el nuevo valor de Y está fuera de los límites, no dibujamos esa
                                                  // celda
                                    if (labels[x][newY] != null) {
                                        labels[x][newY].setOpaque(true);
                                        labels[x][newY].setBackground(Color.RED);
...
=======
    ...
    public static void main(String args[]) {
        ...
        // }
        // } catch (ClassNotFoundException ex) {
        // java.util.logging.Logger.getLogger(Player1.class.getName()).log(java.util.logging.Level.SEVERE,
        // null, ex);
        // } catch (InstantiationException ex) {
        // java.util.logging.Logger.getLogger(Player1.class.getName()).log(java.util.logging.Level.SEVERE,
        // null, ex);
